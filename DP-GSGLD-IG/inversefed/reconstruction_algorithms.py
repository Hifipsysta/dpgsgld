...
class GradientReconstructor():
    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            if self.config['optim'] == 'ADAM':
            ...
            elif self.config['optim'] == 'DPGSGLD':
                param_threshold = 1.0
                scale = np.sqrt(self.config['lr'] * param_threshold * self.config['grad_clip'] / self.config['epsilon'])
                print("======scale=======", scale)
                optimizer = DPGSGLD(params = [x_trial, labels], grad_clip = self.config['grad_clip'], scale = scale, batch_size = 1000, 
                                    device = x_trial.device, lr= self.config['lr'])

            elif self.config['optim'] == 'DPSGD':
                c_2 = 1
                T = 1
                q = 0.5
                sp1 = T * np.log(1/self.config['delta'])
                scale = c_2 * q * np.sqrt(sp1) / self.config['epsilon']

                print("======scale=======", scale)

                C = self.config['grad_clip']

                optimizer = DPSGD(params = [x_trial, labels], C = C, scale = scale*C, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC50':
                optimizer = GCSGD(params = [x_trial, labels], sparsity = 0.5, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC80':
                optimizer = GCSGD(params = [x_trial, labels], sparsity = 0.8, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'DPSGLD':
                N = 6e4
                L = 1
                T = 2*N
                tau = 1000

                sp1 = (128 * N * T * pow(L,2)) / (tau * pow(self.config['epsilon'],2))
                sp2 = np.log((2.5 * N * T) / (tau * self.config['delta']))
                sp3 = np.log(2 / self.config['delta'])
                sp4 = pow(self.config['lr'], 2)
                variance = sp1 * sp2 * sp3 * sp4
                variance = min(variance, self.config['lr'])
                ...
                optimizer = DPSGLD(params = [x_trial, labels], grad_clip = self.config['grad_clip'], scale = scale, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr']) 
            else:
                raise ValueError()
        else:
            if self.config['optim'] == 'ADAM':
                ...
                optimizer = torch.optim.SGD([x_trial], lr=self.config['lr'], momentum=0.9, nesterov=True)
            elif self.config['optim'] == 'LBFGS':
                ...
                scale = np.sqrt(self.config['lr'] * param_threshold * self.config['grad_clip'] / self.config['epsilon'])
                print("======scale=======", scale)
                optimizer = DPGSGLD(params = [x_trial], grad_clip = self.config['grad_clip'], scale = scale, batch_size = 1000, 
                                    device = x_trial.device , lr = self.config['lr'])
                ...
                scale = c_2 * q * np.sqrt(sp1) / self.config['epsilon']
                print("======scale=======", scale)
                optimizer = DPSGD(params = [x_trial], C = self.config['grad_clip'], scale = scale, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC50':
                optimizer = GCSGD(params = [x_trial], sparsity = 0.5, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC80':
                optimizer = GCSGD(params = [x_trial], sparsity = 0.8, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])
                ...
                optimizer = DPSGLD(params = [x_trial], grad_clip = self.config['grad_clip'], scale = scale, batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr']) 
            else:
                raise ValueError()
