    ...
    def _run_trial(self, x_trial, input_data, labels, dryrun=False):
        ...
        if self.reconstruct_label:
            ...
            if self.config['optim'] == 'ADAM':
                ...
                optimizer = torch.optim.SGD([x_trial, labels], lr= self.config['learning_rate'], momentum=0.9, nesterov=True)
            elif self.config['optim'] == 'LBFGS':
                optimizer = torch.optim.LBFGS([x_trial, labels])

            elif self.config['optim'] == 'DPGSGLD':
                optimizer = DPGSGLD(params = [x_trial, labels], grad_clip = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr= self.config['lr'])

            elif self.config['optim'] == 'DPSGD':
                optimizer = DPSGD(params = [x_trial, labels], C = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC50':
                optimizer = GCSGD(params = [x_trial, labels], sparsity = 0.5, scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC80':
                optimizer = GCSGD(params = [x_trial, labels], sparsity = 0.8, scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'DPSGLD':
                optimizer = DPSGLD(params = [x_trial, labels], grad_clip = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr']) 
            else:
                raise ValueError()
        else:
            if self.config['optim'] == 'ADAM':
                ...
                optimizer = DPGSGLD(params = [x_trial], grad_clip = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device , lr = self.config['lr'])

            elif self.config['optim'] == 'DPSGD':
                optimizer = DPSGD(params = [x_trial], C = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC50':
                optimizer = GCSGD(params = [x_trial], sparsity = 0.5, scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'GC80':
                optimizer = GCSGD(params = [x_trial], sparsity = 0.8, scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr'])

            elif self.config['optim'] == 'DPSGLD':
                optimizer = DPSGLD(params = [x_trial], grad_clip = self.config['grad_clip'], scale = self.config['scale'], batch_size = 1000, 
                                    device = x_trial.device, lr = self.config['lr']) 
            else:
                raise ValueError()
