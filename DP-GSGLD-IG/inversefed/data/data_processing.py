...
def construct_dataloaders(dataset, defs, data_path='~/data', shuffle=True, normalize=True):
    ...
    if dataset == 'CIFAR10':
        ...
        trainset, validset = _build_lfw(path, defs.augmentations, normalize)
        loss_fn = Classification()
...
def _build_svhn(data_path, augmentations=True, normalize=True):
    """Define SVHN with everything considered."""
    # Load data
...
def _build_lfw(data_path, augmentations=True, normalize=True):
    """Define SVHN with everything considered."""
    # Load data
    trainset = torchvision.datasets.LFWPeople(root=data_path, split='train', download=True, transform=transforms.ToTensor())
    validset = torchvision.datasets.LFWPeople(root=data_path, split='test', download=True, transform=transforms.ToTensor())

    if lfw_mean is None:
        data_mean, data_std = _get_meanstd(trainset)
    else:
        data_mean, data_std = lfw_mean, lfw_std
    ...
    selected_people = []
    dir_path = os.listdir(r'../data/lfw-py/lfw_funneled/')
    for item in dir_path:
        dir_list = os.path.join(r'../data/lfw-py/lfw_funneled/', str(item))
        if os.path.isdir(dir_list):
            if len(os.listdir(dir_list)) <= 30:
                shutil.rmtree(dir_list, ignore_errors=True)
            else:
