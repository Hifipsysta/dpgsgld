...
parser.add_argument('--learning_rate', type=float, nargs='?', action='store', default=1e-3,
                    help='learning rate for model training.  Default: 1e-2.')

# parser.add_argument('--step_size', type=float, nargs='?', action='store', default=1e-2,
#                     help='stepsize in Langevin dynamics.  Default: 1e-2.')
...
parser.add_argument('--epsilon', type=float, nargs='?', action='store', default =0.05,
...
if args.optimizer == 'DPGSGLD':
    ...
    optimizer = DPSGD(params=model.parameters(), C = C, 
                        scale=scale*C, batch_size = args.batch_size_train, 
    ...
    optimizer = DPSGLD(params=model.parameters(), grad_clip=args.grad_clip, 
                        scale=scale, lr=args.learning_rate/2, 
                        device=device)
...
img_data = gt_data[0]
torchvision.utils.save_image(img_data, 'assets/original_SVHN_'f'{args.img_index}.png') 
    ...
    if (iters+1) % 10 == 0: 
        if iters == args.iterations-1:
            torchvision.utils.save_image(dummy_data[0],
                                        'assets/SVHN_'f'{args.optimizer}_{args.epsilon}_{args.img_index}.png')
...
#plt.axis('off')
#plt.savefig('assets/SVHN_'f'{args.optimizer}_{args.img_index}.png', bbox_inches='tight')
#torchvision.utils.save_image(history[-1],'assets/SVHN_'f'{args.optimizer}_{args.img_index}.png')
