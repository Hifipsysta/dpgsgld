...
parser.add_argument('--epsilon', type=float, nargs='?', action='store', default =2,
...
if args.optimizer == 'DPGSGLD':
    scale = np.sqrt(args.step_size * args.param_threshold * args.grad_clip / args.epsilon)
    ...
    sp4 = pow(args.step_size, 2)
    variance = sp1 * sp2 * sp3 * sp4
    variance = min(variance, args.step_size)
...
for iters in range(args.iterations):
    ...
    if (iters+1) % 10 == 0: 
        if iters == args.iterations-1:
            torchvision.utils.save_image(dummy_data[0],'assets/MNIST_'f'{args.optimizer}_{args.epsilon}_{args.img_index}.png')
